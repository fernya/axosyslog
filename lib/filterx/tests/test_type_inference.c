...
{
  /* meta = {}; meta.a = {}; meta.b = "s";  → meta's *element* type smears to UNKNOWN, but
   * per-key tracking keeps meta.a precisely DICT: a heterogeneous sibling write does not
   * change the fact that meta.a was assigned a dict, so reading meta.a still devirtualizes. */
  FilterXExpr *assign_meta = filterx_assign_new(
                              filterx_floating_variable_expr_new("meta"),
                              filterx_literal_dict_new(NULL));
  FilterXExpr *set_a = filterx_setattr_new(
...
{
  /* meta = {}; meta.x = 5; meta.y = {};  → the int write smears meta's element type to
   * UNKNOWN, but per-key tracking records meta.y as DICT independently, so reading meta.y
   * still devirtualizes (meta.y genuinely holds a dict regardless of the meta.x sibling). */
  FilterXExpr *assign_meta = filterx_assign_new(
                              filterx_floating_variable_expr_new("meta"),
                              filterx_literal_dict_new(NULL));
...
Test(filterx_type_inference, deep_literal_dict_chain_exceeds_old_depth_cap)
{
  /* {"k1":{"k2":{"k3":{"k4":{"k5":{"k6":"leaf"}}}}}} — six dict levels over a string leaf.
   * The previous packed-integer spec capped nesting at depth 4 and truncated levels 5+; the
   * frozen chain has no depth limit, so all six DICT levels and the STRING leaf resolve. */
  FilterXExpr *node = filterx_literal_new(filterx_string_new("leaf", -1));
  for (gint i = 6; i >= 1; i--)
    {
      gchar key[8];
      g_snprintf(key, sizeof(key), "k%d", i);
      GList *elems = g_list_append(NULL, filterx_literal_element_new(
                                     filterx_literal_new(filterx_string_new(key, -1)), node));
      node = filterx_literal_dict_new(elems);
    }
  node = _run(node);

  FilterXStaticTypeSpec spec = node->static_type;
  for (gint level = 0; level < 6; level++)
    {
      cr_assert_eq(filterx_static_type_kind(spec), FILTERX_STATIC_TYPE_DICT,
                   "level %d expected DICT", level);
      spec = filterx_static_type_element(spec);
    }
  cr_assert_eq(filterx_static_type_kind(spec), FILTERX_STATIC_TYPE_STRING);
  filterx_expr_unref(node);
}
