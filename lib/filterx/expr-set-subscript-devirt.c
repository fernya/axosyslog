...
/* The static type that selected this fast path is only a hint, and @typed_set_subscript is
 ...
 * vtable set_subscript has the same signature and serves as the fallback.
 *
                  ...
                  FilterXJITTypedSetSubscript typed_set_subscript, FilterXType *expected_type)
{
  ...
  if (!filterx_object_is_exact_type_or_ref(object, expected_type))
