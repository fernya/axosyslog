...
/* The static type that selected a fast path is only a hint, and @typed_set_subscript is the
 ...
 * set_subscript. @expected_type therefore guards on the exact runtime type. The generic
 * vtable set_subscript has the same signature and serves as the fallback. @expected_type is
 * NULL in the generic helper, which has no fast path and always takes the vtable.
 *
                  ...
                  FilterXJITTypedSetSubscript typed_set_subscript, FilterXType *expected_type)
{
  ...
  if (!expected_type || !filterx_object_is_exact_type_or_ref(object, expected_type))
