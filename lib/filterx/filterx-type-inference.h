 ...
 *
 * Unlike the previous packed-integer representation there is NO depth limit. Frozen nodes
 * live in a process-global, never-freed pool (the set of distinct chains in any config is
 * tiny and deduplicated across reloads), so a spec cached on a FilterXExpr stays valid for
 * as long as that expression does — including at JIT-compile time, after the inference env
 * has been freed. Inference is not on a hot path, so the freezing cost is irrelevant. */
